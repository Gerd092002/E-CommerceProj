    ...
    if ($user_col === 'customerID') {
        if (isset($_SESSION['customerID']) && intval($_SESSION['customerID']) > 0) {
            ...
            if ($custRes && mysqli_num_rows($custRes) > 0) {
                ...
                $userSql = "SELECT userName, email FROM tbl_user WHERE user_id = ? LIMIT 1";
                ...
                if (!$userRow) {
                    ...
                    echo json_encode(['error' => 'Please log in again before adding items to your cart.']);
                    exit();
                }

                $customerName = trim($userRow['userName'] ?? '') ?: 'Customer';
                $customerEmail = trim($userRow['email'] ?? '');
                $emptyValue = '';

                $createCustomerSql = "INSERT INTO tbl_customers (fullname, email, phone, address, user_id) VALUES (?, ?, ?, ?, ?)";
                $createCustomerStmt = $conn->prepare($createCustomerSql);
                $createCustomerStmt->bind_param('ssssi', $customerName, $customerEmail, $emptyValue, $emptyValue, $user_id);
                $createCustomerStmt->execute();

                if ($createCustomerStmt->affected_rows > 0) {
                    $user_val = $conn->insert_id;
                    $_SESSION['customerID'] = $user_val;
                } else {
                    http_response_code(500);
                    echo json_encode(['error' => 'We could not prepare your cart right now. Please try again.']);
                    exit();
                }

                $createCustomerStmt->close();
            }
        }
    }
